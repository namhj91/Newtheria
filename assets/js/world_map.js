...
const renderWorld = (world) => {
  ...
  // 경계 seam(검은 라인) 제거: 텍스처 repeat 패턴 대신 3x3 영역에 헥스를 직접 렌더링한다.
  // 안티앨리어싱/텍스처 경계 샘플링으로 생길 수 있는 1px 줄을 구조적으로 피하기 위한 방식이다.
  const repeatOffsetsX = [0, mapPixelWidth, mapPixelWidth * 2];
  const repeatOffsetsY = [0, mapPixelHeight, mapPixelHeight * 2];
  const HEX_OVERDRAW = 0.38;
  ...
  tiles.forEach((tile) => {
    ...
    repeatOffsetsY.forEach((oy) => {
      repeatOffsetsX.forEach((ox) => {
        // 사용성 주석: 경계 미세 틈을 막기 위해 반지름을 아주 조금 키워서(overdraw) 채운다.
        drawHex(x + ox, y + oy, HEX_CONFIG.size + HEX_OVERDRAW, color);
      });
    });
  });
